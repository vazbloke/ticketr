<template>
  <div>
    ...
    <div class="container">
        ...
        <b-col>
          <b-nav-form class="float-right">
            <b-input-group prepend="Filter by">
              ...
              />
              <b-form-input
                v-if="this.searchSelected=='Ticket ID'"
                v-model="searchValue"
                type="text"
      ...
      <b-row>
        <b-col sm="12">
          <b-alert fade v-model="showDeletedAlert" variant="info" dismissible>Ticket deleted!</b-alert>
        </b-col>
      </b-row>
      <b-card>
        <b-row>
...
export default {
  ...
  methods: {
    getData() {
      let path =
        store.server_url +
        `/ticketdata?&currentPage=${this.currentPage}&limit=${this.limit}` +
        `&sortSelected=${this.sortSelected}&sortOrder=${this.sortOrder}` +
        `&searchSelected=${this.searchSelected}&searchValue=${
          this.searchValue
        }`;
      axios
        .get(path)
